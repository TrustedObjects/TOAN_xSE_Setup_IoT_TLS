...
#define SECURE_STORAGE_NVM_ADDRESS 0x80A0000 // Allow space for TO-Protect with traces
...
#define FLASH_LOG_FUNC(...)    TO_LOG_DBG(__VA_ARGS__)
...
TO_lib_ret_t HAL_nvm_secure_storage_init(void)
{
	...
	if (ret == 0) {
		...
		FLASH_LOG_ERR("flash_secure_storage.init() returned %x", (unsigned int)ret);
		return TO_ERROR;
	}
...
TO_lib_ret_t HAL_nvm_secure_storage_deinit(void)
{
	...
	if (ret == 0) {
		...
		FLASH_LOG_ERR("flash_secure_storage.deinit() returned %x", (unsigned int)ret);
		return TO_ERROR;
	}
...
TO_lib_ret_t TODRV_SSE_secure_storage_load(uint8_t *dst, uint32_t src_offset, uint32_t size)
{
	...
	FLASH_LOG_FUNC ("  >>> TODRV_SSE_secure_storage_load (src_offset=%08x, size=%d", src_offset, size);

	if ((src_offset + size) > SECURE_STORAGE_NVM_SIZE) {
		FLASH_LOG_ERR("Bad offset 0x%x or size %u", (unsigned)src_offset, (unsigned)size);
		return TO_ERROR;
	}
...
TO_lib_ret_t TODRV_SSE_secure_storage_store(uint32_t dst_offset, const uint8_t *src, uint32_t size)
{
	...
	FLASH_LOG_FUNC(">>> TODRV_SSE_secure_storage_store(dst_offset=%08x, size=%u", (unsigned)dst_offset, (unsigned)size);
	...
	if (sector_size != TODRV_SSE_NVM_SECTOR_SIZE) {
		...
		FLASH_LOG_ERR("TODRV_SSE_NVM_SECTOR_SIZE=%d; flash_secure_storage.get_sector_size() says: %d", TODRV_SSE_NVM_SECTOR_SIZE, (int)sector_size);
		return TO_MEMORY_ERROR;
	}
	FLASH_LOG_DBG("sector_size=%d", sector_size);
	...
	if ((ret = flash_secure_storage.erase((uint32_t)addr_flash_secure_storage, program_size)) != 0) {
		FLASH_LOG_ERR("Failed to erase sector at offset 0x%p with error %d", addr_flash_secure_storage, ret);
		goto memory_error;
	}

	if ((ret = flash_secure_storage.program(src, addr_flash_secure_storage, program_size)) != 0) {
		FLASH_LOG_ERR("Failed to write %d bytes at offset 0x%08x with error %d", (int)sector_size, addr_flash_secure_storage, ret);
		goto memory_error;
	} else {
	...
	if ((ret = memcmp((void *)in_address, in_data, in_size)) != 0) {
		FLASH_LOG_ERR("memcmp returns %d - Wrote data differs (in size=%d)", ret, size);
